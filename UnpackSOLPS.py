...
class SOLring:
    def __init__(
        ...
        self.radTrans = (
            radTrans - radTrans[-1]
        )  # heat flux loss due to radial transport
        self.FlowVelocity = FlowVelocity
        self.recombSource = recombSource
        self.ionisSource = ionisSource
        self.frontPositions = []
        self.heatsTotal = 0
        self.coolingCurve = interpolate.interp1d(
            self.te, self.qf / self.ne**2, kind="cubic"
        )
        self.volumetricSinks = (
            -1 * self.ionisLoss - self.recombLoss - self.molecularloss - self.radTrans
        )  # +self.divuloss
    ...
    def calcFrontTemp(self, TempPoint):
        ...
        if np.amin(self.te) < TempPoint:
            ...
            sfunc = interpolate.interp1d(
                self.te, self.Spar, kind="cubic", bounds_error=False, fill_value=0
            )
    ...
    def calcFrontqpll(self):
        ...
        if frontIndex > 0:
            if np.amin(gradq[: frontIndex + 1]) < np.amax(gradq) * perc:
                lowerFunc = interpolate.interp1d(
                    gradq[: frontIndex + 1], stest[: frontIndex + 1]
                )
    ...
    def calcFrontqpll(self):
        ...
        qoverB = interpolate.interp1d(
            stest, self.cond / self.B, fill_value="extrapolate", bounds_error=False
        )
        diff = 0.01

        for windowlength in np.linspace(
            diff, np.amax(stest), int((np.amax(stest) - diff) / diff)
        ):
            heatchange = []
            ...
            if np.amax(heatchange) >= qoverB(np.amax(stest)) / 2:
                ...
                return (
                    stest[np.argmax(gradqoverB)],
                    s0s[windowindex] + windowlength,
                    s0s[windowindex],
                )
    ...
    def returnFrontPosition(self):
        return np.mean(self.frontPositions), np.amax(self.frontPositions) - np.amin(
            self.frontPositions
        )

    def returnCalculationsC(self, frontpos, xindex):
        # 0 is most theoretical
        Tinterp = interpolate.interp1d(self.te, self.Spar, kind="cubic")
        Lfunc = []
        for t in self.te:
            Lfunc.append(LfuncN(t))
        Lfunc = np.array(Lfunc)
        kappa0 = 2600
        fi = 0.03
        # kappa = 1000
        FieldInterp = interpolate.interp1d(
            self.Spar,
            self.B,
            kind="cubic",
            bounds_error=False,
            fill_value=(self.B[0], self.B[-1]),
        )

        Coldindex = 0
        Calc0_term0 = (self.cond[xindex] ** 2) / (2 * fi * self.B[xindex] ** 2)
        Calc0 = Calc0_term0 / (self.te[-1] ** 2)
        int0 = kappa0 * trapz(
            FieldInterp(frontpos) ** (-2)
            * Lfunc[Coldindex:]
            * self.te[Coldindex:] ** (1 / 2),
            self.te[Coldindex:],
        )
        Calc0 = Calc0 / int0
        Calc0 = np.sqrt(Calc0)

        # calculation 1 removes assumption of thin front
        Calc1 = Calc0_term0 / (self.te[-1] ** 2)
        int1 = kappa0 * trapz(
            self.B ** (-2) * Lfunc[Coldindex:] * self.te[Coldindex:] ** (1 / 2),
            self.te[Coldindex:],
        )
        Calc1 = Calc1 / int1
        Calc1 = np.sqrt(Calc1)

        # calculation 2 removes assumption of conductivity

        Calc2 = Calc0_term0
        int2 = trapz(
            self.B ** (-2) * Lfunc[Coldindex:] * self.qpar[Coldindex:],
            self.Spar[Coldindex:],
        )
        Calc2 = Calc2 / int2
        Calc2 = np.sqrt(Calc2)
        ...
        return [Calc0, Calc1, Calc2, Calc3]
...
def unpack2d(rootgrp):
    ...
    # grid coordinates:
    quantities2d["r"] = (
        rootgrp["crx"][0] + rootgrp["crx"][1] + rootgrp["crx"][2] + rootgrp["crx"][3]
    ) / 4
    quantities2d["z"] = (
        rootgrp["cry"][0] + rootgrp["cry"][1] + rootgrp["cry"][2] + rootgrp["cry"][3]
    ) / 4
    hx = rootgrp["hx"]
    ...
    quantities2d["recombSource"] = (
        np.sum(rootgrp["eirene_mc_pppl_sna_bal"], axis=0)[1] / dv
    )
    ...
    quantities2d["ionisSource"] = (
        np.sum(rootgrp["eirene_mc_papl_sna_bal"], axis=0)[1] / dv
    )
    # convected electron heat flux (Wm^{-2})
    fhe_conv = (
        te * np.array(rootgrp["ne"]) * 5
        + 0.5 * 1.67 * 10 ** (-27) * np.array(rootgrp["ne"]) * vfluid**2
    ) * vfluid
    ...
    quantities2d["ionisationloss"] = np.sum(
        rootgrp["eirene_mc_eael_she_bal"], axis=0
    )  # 13.6*1.602E-19*rootgrp["b2stel_sna_ion_bal"][0]
    ...
    quantities2d["recombloss"] = np.sum(
        rootgrp["eirene_mc_epel_she_bal"], axis=0
    )  # np.sum(rootgrp['eirene_mc_eiel_she_bal'],axis=0)
    # plt.plot(quantities2d["te"].flatten(),quantities2d["recombloss"].flatten(),linestyle="",marker = "o")
    # plt.show()
    # heat loss due to div of atom velocity
    quantities2d["Divuloss"] = np.array(rootgrp["b2sihs_divua_bal"]) + np.array(
        rootgrp["b2sihs_divue_bal"]
    )

    quantities2d["radialHeatFlux"] = (
    ...
    quantities2d["iHeat"] = (
        ...
        + rootgrp["fhi_dia"][0]
        + rootgrp["fhi_ecrb"][0]
        + rootgrp["fhi_strange"][0]
        + rootgrp["fhi_pschused"][0]
        + rootgrp["fhi_inert"][0]
        + rootgrp["fhi_vispar"][0]
        + rootgrp["fhi_visper"][0]
        + rootgrp["fhi_visq"][0]
        + rootgrp["fhi_anml"][0]
        + rootgrp["fhi_kevis"][0]
    )
    quantities2d["iHeat"] = quantities2d["iHeat"]
    quantities2d["qpar"] = (quantities2d["iHeat"] + quantities2d["elHeat"]) / (
        quantities2d["Area"]
    )
    # charge exchange momentum loss
    quantities2d["electronThermalForce"] = np.sum(
        np.array(rootgrp["b2sifr_smotfea_bal"]), axis=0
    )
...
def unpackSOLPS(fileName, reverse, ring, Xpoint=-1):
    ...
    # plt.show()
    SOLring1 = SOLring(
        Spar=np.cumsum(quantities2d["sdiff"][ring][::reverse][1:Xpoint]),
        ...
        n0rad=(quantities2d["neutralrad"] / quantities2d["V"])[ring][::reverse][
            1:Xpoint
        ],
        ionisLoss=(quantities2d["ionisationloss"] / quantities2d["V"])[ring][::reverse][
            1:Xpoint
        ],
        recombLoss=(quantities2d["recombloss"] / quantities2d["V"])[ring][::reverse][
            1:Xpoint
        ],
        molecularloss=(quantities2d["molecularloss"] / quantities2d["V"])[ring][
            ::reverse
        ][1:Xpoint],
        divuloss=(quantities2d["Divuloss"] / quantities2d["V"])[ring][::reverse][
            1:Xpoint
        ],
        conv=-1 * reverse * quantities2d["conv"][ring][::reverse][1:Xpoint],
        radTrans=(
            (
                quantities2d["radialHeatFlux"][ring + 1]
                - quantities2d["radialHeatFlux"][ring]
            )
            / quantities2d["V"][ring]
        )[::reverse][1:Xpoint],
        ...
        electronThermalForce=(quantities2d["electronThermalForce"])[ring][::reverse][
            1:Xpoint
        ],
    )
