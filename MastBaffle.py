...
def readWallflx(fname, targtype):
    ...
    for i in range(1, len(rootgrp["crx"][1]) - 1):
        A.append(
            np.sqrt(
                (rootgrp["crx"][1][i][-2] - rootgrp["crx"][3][i][-2]) ** 2
                + (rootgrp["cry"][1][i][-2] - rootgrp["cry"][3][i][-2]) ** 2
            )
        )
...
def pressureBalance(rootgrp):
    ...
    for i in range(ny):
        for j in range(nx):
            if leftix[i, j] < 1:
                continue
            apllx[i, j] = (
                apll[leftiy[i, j], leftix[i, j]] * dv[i, j]
    ...
    fmoy_flua = (
        np.sum(np.array(rootgrp["fmo_flua"]), axis=0)[1]
        + np.sum(np.array(rootgrp["fmo_cvsa"]), axis=0)[1]
    )
    for i in range(ny):
        for j in range(nx):
            if topiy[i, j] > ny:
    ...
    # b2 neutral sinks
    b2stel_smq_ion_bal = np.sum(
        np.array(rootgrp["b2stel_smq_ion_bal"])[ion_indices], axis=0
    )
...
def CalculateSinks(rootgrp):
    ...
    ion_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_eapl_shi_bal"]), axis=0)
    )
    ion_heat["labels"].append("eirene_mc_eapl_shi")

    ion_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_empl_shi_bal"]), axis=0)
    )
    ion_heat["labels"].append("eirene_mc_empl_shi_bal")

    ion_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_eipl_shi_bal"]), axis=0)
    )
    ion_heat["labels"].append("eirene_mc_eipl_shi_bal")

    ion_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_eppl_shi_bal"]), axis=0)
    )
    ...
    elec_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_eael_she_bal"]), axis=0)
    )
    elec_heat["labels"].append("eirene_mc_eael_she_bal")

    elec_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_emel_she_bal"]), axis=0)
    )
    elec_heat["labels"].append("eirene_mc_emel_she_bal")

    elec_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_eiel_she_bal"]), axis=0)
    )
    elec_heat["labels"].append("eirene_mc_eiel_she_bal")

    elec_heat["sinks"].append(
        np.sum(np.array(rootgrp["eirene_mc_epel_she_bal"]), axis=0)
    )
...
def heatBalance(fname, rootgrp, quantities2d, XPTs, heatmode, heat_counts):
    ...
    innertarHeat = (
        np.sum(b2stbc_she_bal[1:-1, 0])
        + np.sum(b2stbc_shi_bal[1:-1, 0])
        + np.sum(b2stbc_she_bal[1:-1, XPTs[2] - 1])
        + np.sum(b2stbc_shi_bal[1:-1, XPTs[2] - 1])
    )

    outertarHeat = (
        np.sum(b2stbc_she_bal[:, XPTs[2]])
        + np.sum(b2stbc_shi_bal[:, XPTs[2]])
        + np.sum(b2stbc_she_bal[:, -1])
    ...
    totalsink_Core = np.sum(
        totalsink[: rootgrp["jsep"][0] + 2, XPTs[0] : XPTs[1]]
    ) + np.sum(totalsink[: rootgrp["jsep"][0] + 2, XPTs[3] : XPTs[4]])
    totalsink_SOL = np.sum(
        totalsink[rootgrp["jsep"][0] + 2 :, XPTs[0] : XPTs[1]]
    ) + np.sum(totalsink[rootgrp["jsep"][0] + 2 :, XPTs[3] : XPTs[4]])
    totalsink_Div = (
        np.sum(totalsink[:, : XPTs[0]])
        + np.sum(totalsink[:, XPTs[1] : XPTs[3]])
    ...
    if heatmode == "source_type":
        ...
        heat_counts["hydrogen"].append(
            np.abs(np.sum(atomrad) + np.sum(molrad) + np.sum(eionrad)) / radsource
        )
        ...
        heat_counts["main chamber SOL radiation"].append(
            np.abs(totalsink_SOL) / radsource
        )
        heat_counts["divertor radiation"].append(np.abs(totalsink_Div) / radsource)
        heat_counts["non-radiative targets"].append(
...
def perform_Analysis(power, simFiles, collisionality=False):
    ...
    for folder in folderList:
        ...
        for File in Files:
            ...
            if colornum == 2:
                cummDloss = -1 * np.cumsum(
                    (SOLring1.ionisLoss + SOLring1.recombLoss) * SOLring1.V
                )
            ...
            if SOLring1.te[0] < 30:
                ...
                totalpress = (
                    SOLring1.te + SOLring1.ti + SOLring1.FlowVelocity**2
                ) * SOLring1.ne
                fmom.append(
                    (SOLring1.te[0] + SOLring1.ti[0])
                    * SOLring1.ne[0]
                    / ((SOLring1.te[-1] + SOLring1.ti[-1]) * SOLring1.ne[-1])
                )
            Qu.append(
                quantities2d["qpar"][RING][XPTs[-1]]
                * quantities2d["Area"][RING][XPTs[-1]]
            )
            QF.append(imprad[-1])
            ...
            if not detached:
                if SOLring1.te[0] < 5:
                    ...
                    ionisloss = np.sum(
                        ionisloss[SEPARATRIX:-1, midplaneix : XPTs[-1]], axis=1
                    )
                    ionisloss = (
                        np.abs(ionisloss)
                    ...
                    radheataxs.plot(
                        Rrsep[1:-1],
                        1e-6 * radheat[2:, midplaneix],
                        color=heatcolor,
                        marker=markerstyle,
                        label=templabel + " radial heat flux",
                    )
                    radheataxs.plot(
                        Rrsep[1:-1],
                        1e-6
                        * (
                        ...
                        color=heatcolor,
                        linestyle="--",
                        label=templabel + " conducted radial heat flux",
                    )

                    # plot radial temperature profile at the midplane
                    upstempaxs.plot(
                        Rrsep[SEPARATRIX - 1 : -1],
                        quantities2d["te"][SEPARATRIX - 1 : -1, midplaneix],
                        color=heatcolor,
                        marker=markerstyle,
                        label=templabel + " radial heat flux",
                    )
                    ...
                    radtransloss = (
                        -1
                        * np.cumsum(
                            radheat[RING, XPTs[4] - 1 : -2]
                            - radheat[RING + 1, XPTs[4] - 1 : -2]
                        )[::-1]
                    )
                    ...
                    radtransloss = (
                        -1
                        * np.cumsum(
                            radheat[RING, midplaneix - 1 : -2]
                            - radheat[RING + 1, midplaneix - 1 : -2]
                        )[::-1]
                    )
                    ...
                    maximp = np.max(
                        -1
                        * np.cumsum(quantities2d["imprad"][RING, midplaneix:-1])[::-1]
                    )
                    maxradTrans = np.max(
                        -1
                        * np.cumsum(
                            radheat[RING - 1, midplaneix:-1]
                            - radheat[RING, midplaneix:-1]
                        )[::-1]
                    )
                    maxionis = np.max(
                        -1
                        * np.cumsum(
                            np.sum(rootgrp["eirene_mc_eael_she_bal"], axis=0)[
                                RING, XPTs[4] : -1
                            ]
                        )[::-1]
                    )
        ...
        if "Tight" in folder:
            ...
            if collisionality:
                axs1.plot(
                    collis, frontpos, marker="o", label=label, color=gridcolors[2]
                )
                ax12.plot(
                    collis,
                    peakheatLoad * 10 ** (-6),
    ...
    if collisionality:
        ...
        fig1.savefig(
            "Figures/BaffleFluxEvolution" + power + ".png", dpi=800, bbox_inches="tight"
        )  #
    else:
        fig1.savefig(
            "Figures/BaffleEvolution" + power + ".png", dpi=800, bbox_inches="tight"
        )  #
    ...
    axs0.legend()
    fig0.savefig(
        "Figures/Baffle_Profiles/baffle_Heat_faloff" + power + ".png",
        dpi=800,
        bbox_inches="tight",
    )
    ...
    fig4.savefig(
        "Figures/Baffle_Profiles/baffle_densUpstream" + power + ".png",
        dpi=800,
        bbox_inches="tight",
    )
    ...
    axs5.legend()
    fig5.savefig(
        "Figures/Baffle_Profiles/baffle_TempUpstream" + power + ".png",
        dpi=800,
        bbox_inches="tight",
    )
    ...
    fig6.tight_layout()
    fig6.savefig(
        "Figures/Baffle_Profiles/upstream_particle_flux" + power + ".png",
        dpi=800,
        bbox_inches="tight",
    )
    ...
    radheatfig.savefig(
        "Figures/Baffle_Profiles/radHeatflux" + power + ".png",
        dpi=800,
        bbox_inches="tight",
    )
    ...
    radheatfig.tight_layout()
    upstempfig.savefig(
        "Figures/Baffle_Profiles/radTempUpstream" + power + ".png",
        dpi=800,
        bbox_inches="tight",
    )
...
def plotEnergyCost(simFiles, power):
    ...
    cb1 = mpl.colorbar.ColorbarBase(
        ...
        norm=norm,
        label="ionisation source [m" + r"$^{-3}$" + "s" + r"$^{-1}$" + "]",
    )
...
def plotClosure(power):
    ...
    plt.savefig(
        "Figures/Baffle_Profiles/" + power + "closure.png",
        dpi=1000,
        bbox_inches="tight",
    )
...
def plotLosses(power, degDetachment):
    ...
    for fileName in files:
        ...
        totalsink = np.abs(
            np.sum(ion_heat["sinks"], axis=0) + np.sum(elec_heat["sinks"], axis=0)
        )
        Rrsep = 1000 * (
        ...
        totalsink = (
            10 ** (-6)
            * np.abs(np.array(rootgrp["b2stel_she_bal"][1]))
            / quantities2d["V"]
        )
        totalsink = 10 ** (-3) * np.abs(np.array(rootgrp["ua"][1]))
        totalsink = np.abs(np.array(rootgrp["ua"][1])) / np.sqrt(
            (quantities2d["ti"] + quantities2d["te"]) * 1.6e-19 / (2 * 1.67e-27)
        )
...
def plotEvolution(power):
    ...
    for folder in [
        ...
        m = matplotlib.cm.ScalarMappable(
            norm=mpl.colors.Normalize(vmin=0, vmax=len(files)), cmap=cmap
        )
        counter = 0
        ...
        for file in files:
            ...
            CUMSUM = -1 * np.cumsum(
                np.sum(
                    np.array(rootgrp["b2stel_she_bal"])[1][SEPARATRIX:-1, midplaneix:],
                    axis=0,
                )
            )
            ...
            label = (
                ...
                + r"$_{u}$"
                + "="
                + str(int(quantities2d["ne"][RING][midplaneix] * 10 ** (-18)) / 10)
                + r"$\times 10^{19}$"
                + " m"
                + r"$^{-3}$"
            )
            conv = totrad - cond
            contaxs[axsnum].contour(
                quantities2d["r"],
                quantities2d["z"],
                quantities2d["te"],
                [10],
